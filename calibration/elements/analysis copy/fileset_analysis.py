...
class FileSetAnalysis(BaseAnal):
    ...
    def analyze_pedestals(self):
        ...
        self.results['pedestals'] = {
            ...
            'full_set':{
                'RefPD': self.fs.def_pedestal['meanRefPD'].mean(),
                'PM': self.fs.def_pedestal['meanPM'].mean(),
                'RefPD_std': self.fs.def_pedestal['meanRefPD'].std(),
                'PM_std': self.fs.def_pedestal['meanPM'].std()
            }
        }
    ...
    def _gen_pedestals_timeseries_plot(self):
        ...
        x = range(len(self.fs.def_pedestal))

        # ─────────────────────────────
        # Top plot: PM pedestals
        # ─────────────────────────────
        ax1.errorbar(
            self.fs.def_pedestal['datetime'], self.fs.def_pedestal['meanPM'],
            yerr=self.fs.def_pedestal['stdPM'],
            fmt='.', markersize=10, linewidth=1,
            label='PM Pedestals'
        )
        ax1.set_ylabel('Pedestals PM (W)')
        ...
        # ─────────────────────────────
        ax2.errorbar(
            self.fs.def_pedestal['datetime'], self.fs.def_pedestal['meanRefPD'],
            yerr=self.fs.def_pedestal['stdRefPD'],
            fmt='.', markersize=10, linewidth=1,
            label='RefPD Pedestals'
        )
        ax2.set_ylabel('Pedestals RefPD (V)')
